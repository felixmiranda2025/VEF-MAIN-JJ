...
// ================================================================
// ================================================================
// IA — Asistente para cotizaciones (Anthropic Claude)
// ================================================================
app.get('/api/ia/estado', auth, async (req, res) => {
  try {
    const schema = req.user?.schema || 'emp_vef';
    // Contar cotizaciones existentes como "ejemplos"
    const rows = await QR(req, 'SELECT COUNT(*) cnt FROM cotizaciones WHERE total > 0');
    ...
    res.json({
      ...
      mensaje:         anthropic
        ? `IA lista. ${cnt} cotizaciones como referencia.`
        : 'Configura ANTHROPIC_API_KEY en el .env para activar la IA.'
    });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/ia/generar-cotizacion', auth, async (req, res) => {
...
app.post('/api/ia/mejorar-descripcion', auth, async (req, res) => {
